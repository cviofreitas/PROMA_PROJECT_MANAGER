...
type NewServiceFormMobileProps = {
  projectId: number;
  client: Client;
  project: Project | undefined;
  setEditMode: (value: boolean) => void
}
const NewServiceFormMobile = ({ projectId, client, project, setEditMode }: NewServiceFormMobileProps) => {
    ...
    <div className="PopUpBackground">
      <form className="InfoForm " onSubmit={handleSubmit}>
        ...
        <div className="MarginTop10">
          <h6>Amount</h6>
          <h5>{formatterUSD.format(Number(formData.qty) * Number(formData.rate))}</h5>
        </div>
        <div className="MarginTop10">
          <Button className="SaveButton" onClick={handleSubmit}>
            Add
          </Button>
          <Button className="SaveButton" onClick={() => setEditMode(false)}>
            Cancel
          </Button>
        </div>
      </form>
